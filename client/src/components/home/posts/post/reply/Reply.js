...
function Reply({ reply, post, posts, setPosts }) {
  ...
  const closeAddReplyToReply = () => {
    setReplyToReplyError('')
    setReplyToReply('')
    setDisplayAddReplyToReply(false)
  }
  ...
  const addReplyToReply = () => {
    ...
    fetch('/posts/addReply', {
      ...
      .then(reply => {
        ...
        if (reply.err) return setReplyToReplyError(reply.err)
  ...
  const [replyToReplyError, setReplyToReplyError] = useState('')

  const updateReply = () => {
    if (reply.reply === editedReply) return
